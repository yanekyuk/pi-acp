...
import { cleanTitle, deriveFallbackTitle, buildTitlePrompt, generateTitle } from '../../src/acp/title.js'
...
test('buildTitlePrompt: formats conversation context', () => {
  const prompt = buildTitlePrompt({
    userMessage: 'Help me migrate from Vue 2 to Vue 3',
    assistantMessage: 'Here is a step-by-step migration guide'
  })
  assert.match(prompt, /Generate a concise, descriptive title/)
...
test('generateTitle: falls back when spawning throws synchronously', async () => {
  ...
  assert.equal(title, 'Investigate flaky integration tests')
})

test('generateTitle: falls back to deriveFallbackTitle on spawn error', async () => {
  const mockSpawn = ((_cmd: string, _args: string[]) => {
    const child = new EventEmitter() as any
    child.stdout = new EventEmitter()
    child.kill = () => {}
  ...
  assert.equal(title, 'Setup Redis connection pooling')
})
